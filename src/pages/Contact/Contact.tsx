...
import { ArrowRight, Mail, MapPin, MessageCircle, Phone, Send, Plus, X } from 'lucide-react'
...
export function Contact(){
 ...
 const addProduct=()=>{if(product&&!selectedProducts.includes(product)){setSelectedProducts(current=>[...current,product]);setProduct('')}}
 const removeProduct=(item:string)=>setSelectedProducts(current=>current.filter(value=>value!==item))
 const submit=(event:React.FormEvent)=>{event.preventDefault();const lines=['*New Product Enquiry*','', '*Customer Details*', 'Name: '+form.name, 'Company: '+(form.company||'Not provided'), 'Phone: '+form.phone, 'Email: '+(form.email||'Not provided'), 'Country: '+(form.country||'Not provided'), '', '*Requirement*', 'Category: '+(category?.title||'Not selected'), 'Products: '+(selectedProducts.length?selectedProducts.join(', '):'Not selected'), 'Quantity: '+(form.quantity||'Not specified'), 'Message: '+(form.message||'No additional message')];window.open('https://wa.me/'+company.whatsappNumber+'?text='+encodeURIComponent(lines.join('\n')),'_blank','noopener,noreferrer')}
 ...
 <section className="contact-page"><div className="contact-page__intro"><span className="section-label">BUSINESS INQUIRIES</span><h2>Tell us what you are <em>looking for.</em></h2><p>Select products from our catalogue, share your requirement, and send the complete enquiry directly to WhatsApp.</p><div className="contact-page__quicklinks"><a href={'mailto:'+company.email}><Mail size={18}/><div><small>EMAIL</small><strong>{company.email}</strong></div><ArrowRight size={16}/></a>{company.phoneNumbers.map(person=><a key={person.number} href={'tel:+91'+person.number}><Phone size={18}/><div><small>{person.name.toUpperCase()}</small><strong>+91 {person.number}</strong></div><ArrowRight size={16}/></a>)}</div></div>
 ...
 <div className="contact-product-picker"><label>Product<select disabled={!category} value={product} onChange={e=>setProduct(e.target.value)}><option value="">{category?'Select a product':'Select category first'}</option>{category?.products.filter(item=>!selectedProducts.includes(item)).map(item=><option key={item}>{item}</option>)}</select></label><button type="button" className="contact-product-add" onClick={addProduct} disabled={!product}><Plus size={17}/> Add</button></div><div className="contact-selected-products">{selectedProducts.length===0?<span>Select products and press + Add to include multiple products.</span>:selectedProducts.map(item=><button type="button" key={item} onClick={()=>removeProduct(item)}>{item}<X size={13}/></button>)}</div>
 ...
 <input type="hidden" required value={selectedProducts.length?selectedProducts.join(', '):''} onChange={()=>{}}/><button type="submit" className="contact-enquiry__submit"><Send size={17}/> Send enquiry on WhatsApp <ArrowRight size={17}/></button></form></section></>
}